...
export function ColumnCard({ column, index, count }: { column: Column; index: number; count: number }) {
  ...
  return (
    <div className="card">
      ...
      <SelectField
        label={t('ui.interior')}
        value={column.interior}
        options={[{ value: 'shelves', label: t('ui.interior.shelves') }, { value: 'drawers', label: t('ui.interior.drawers') }]}
        onChange={(v) => set({ interior: v })}
      />
      {column.interior === 'drawers' ? (
      ...
      <CheckField label={t('ui.door')} value={column.door} onChange={(v) => set({ door: v })} />
