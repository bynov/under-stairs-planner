...
import { FLAT_ROT, NO_ROT, ROD_ROT, SIDE_ROT, toWorld, v3, type Vec3 } from './vec';

// The 3D viewport (src/ui/three/PartMesh.tsx) builds a THREE.Matrix4 from a part's
// {position, rotation} transform independently of toWorld(). This test pins the two
// implementations together so they can never silently diverge.
...
describe('three.js matrix matches toWorld()', () => {
  const rotations: Record<string, Vec3> = {
    NO_ROT, FLAT_ROT, SIDE_ROT, ROD_ROT,
    SLOPED_TOP: v3(Math.PI / 2, 0, -Math.atan(0.5)),
  };
